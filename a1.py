...
class NeuralNet:
    ...
    def train( self,X,Y,numepochs = 100,learning_rate = 0.1,initialization_type = "random", activation = "sigmoid",optimiser = "vanilla",gamma=0.1):
        ...
        for epoch in tqdm(range(numepochs)):
            grads = {}
            acc = self.prefict(X,returnclass=1)
...
wandb.config = {"dataset":"fashion_mnist","input_size":784,"output_size":10,"hidden_layers":[20,20,10],"epochs":60,\
                     "learning_rate":2.5,"batch_size":None,"initialization_type":"random","activation":"sigmoid","optimiser":"nesterov","gamma":0.1}
...
nn.train(X_train,Y_train,wandb.config["epochs"],wandb.config["learning_rate"],\
         initialization_type=wandb.config["initialization_type"],activation=wandb.config["activation"],optimiser=wandb.config["optimiser"],gamma=wandb.config["gamma"])
